...
COLOR_TO_TRACK = {"lower_range": np.array([139,0,0]),
                  "upper_range": np.array([225, 204, 203])} # the rgb range for whatever color

object_model = YOLO("C:/Users/Administrator/PycharmProjects/Kinect_body_tracking_v2/runs/detect/train/weights/best.pt")
...
while True:
    ...
    results = object_model.predict(newest_frame, verbose = False)
    try:
        for result in results: #just gives us access to all the data.
            for box in result.boxes:
                ...
                if confidence >.50 and check_for_color(newest_frame, x1, y1, x2, y2, COLOR_TO_TRACK["lower_range"], COLOR_TO_TRACK["upper_range"]):
